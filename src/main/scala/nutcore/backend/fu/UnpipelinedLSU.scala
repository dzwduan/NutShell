...
class UnpipelinedLSU extends NutCoreModule with HasLSUConst {
  ...
  def access(valid: Bool, src1: UInt, src2: UInt, func: UInt, dtlbPF: Bool): UInt = {
    ...
    BoringUtils.addSource(WireInit(setLr), "set_lr")
    BoringUtils.addSource(WireInit(setLrVal), "set_lr_val")
    BoringUtils.addSource(WireInit(setLrAddr), "set_lr_addr")
...
class LSExecUnit extends NutCoreModule {
  ...
  BoringUtils.addSource(WireInit(BoolStopWatch(dmem.isRead(), dmem.resp.fire)), "perfCntCondMloadStall")
  BoringUtils.addSource(WireInit(BoolStopWatch(dmem.isWrite(), dmem.resp.fire)), "perfCntCondMstoreStall")
  BoringUtils.addSource(WireInit(io.isMMIO), "perfCntCondMmmioInstr")
}
