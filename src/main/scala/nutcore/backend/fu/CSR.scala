...
class CSR(implicit val p: NutCoreConfig)
    extends NutCoreModule
    with HasCSRConst {
  val io = IO(new CSRIO)

  val (valid, src1, src2, func) =
    (io.in.valid, io.in.bits.src1, io.in.bits.src2, io.in.bits.func)
  ...
  val misaInitVal = getMisaMxl(2) | extList.foldLeft(0.U)((sum, i) =>
    sum | getMisaExt(i)
  ) // "h8000000000141105".U
  ...
  val mhartid =
    RegInit(UInt(XLEN.W), 0.U) // the hardware thread running the code
  ...
  val perfCntsLoMapping = (0 until nrPerfCnts).map { case i =>
    MaskedRegMap(0xb00 + i, perfCnts(i))
  }
  val perfCntsHiMapping = (0 until nrPerfCnts).map { case i =>
    MaskedRegMap(0xb80 + i, perfCnts(i)(63, 32))
  }
  ...
  val justRead =
    (func === CSROpType.set || func === CSROpType.seti) && src1 === 0.U // csrrs and csrrsi are exceptions when their src1 is zero
  val isIllegalWrite =
    wen && (addr(
      11,
      10
  ...
  val ret = Wire(Bool())
  val isEbreak =
    addr === privEbreak && func === CSROpType.jmp && !io.isBackendException
  val isEcall =
    addr === privEcall && func === CSROpType.jmp && !io.isBackendException
  val isMret =
    addr === privMret && func === CSROpType.jmp && !io.isBackendException
  val isSret =
    addr === privSret && func === CSROpType.jmp && !io.isBackendException
  val isUret =
    addr === privUret && func === CSROpType.jmp && !io.isBackendException

  Debug(
    wen,
    "csr write: pc %x addr %x rdata %x wdata %x func %x\n",
    io.cfIn.pc,
    addr,
    rdata,
    wdata,
  ...
  io.dmemMMU.priviledgeMode := Mux(
    mstatusStruct.mprv.asBool,
    mstatusStruct.mpp,
    priviledgeMode
  )
  ...
  when(hasInstrPageFault || hasLoadPageFault || hasStorePageFault) {
    val tval = Mux(
      ...
      Mux(
        ...
        SignExt((io.cfIn.pc + 2.U)(VAddrBits - 1, 0), XLEN),
        SignExt(io.cfIn.pc(VAddrBits - 1, 0), XLEN)
      ),
      SignExt(dmemPagefaultAddr, XLEN)
    )
    ...
    Debug(
      "[PF] %d: ipf %b tval %x := addr %x pc %x priviledgeMode %x\n",
      GTimer(),
      hasInstrPageFault,
      tval,
      SignExt(dmemPagefaultAddr, XLEN),
  ...
  when(hasLoadAddrMisaligned || hasStoreAddrMisaligned) {
    ...
    Debug(
      "[ML] %d: addr %x pc %x priviledgeMode %x\n",
      GTimer(),
      SignExt(dmemAddrMisalignedAddr, XLEN),
      io.cfIn.pc,
      priviledgeMode
    )
  }
  ...
  intrVecEnable.zip(ideleg.asBools).map { case (x, y) =>
  ...
  csrExceptionVec(
    illegalInstr
  ) := (isIllegalAddr || isIllegalAccess) && wen && !io.isBackendException // Trigger an illegal instr exception when unimplemented csr is being read/written or not having enough priviledge
  ...
  val tvalWen =
    !(hasInstrPageFault || hasLoadPageFault || hasStorePageFault || hasLoadAddrMisaligned || hasStoreAddrMisaligned) || raiseIntr // in nutcore-riscv64, no exception will come together with PF
  ...
  val perfCntList =
    generalPerfCntList ++ (if (EnableOutOfOrderExec) outOfOrderPerfCntList
                           else sequentialPerfCntList)
  ...
  if (!p.FPGAPlatform) {
    ...
    if (hasPerfCnt) {
      ...
      when(nutcoretrap) {
        ...
        if (PrintPerfCntToCSV) {
          printf("======== PerfCntCSV =========\n\n")
          perfCntList.toSeq.sortBy(_._2._1).map {
            case (name, (addr, boringId)) =>
              printf(name + ", ")
          }
          printf("\n\n\n")
    ...
    if (EnableDifftest) {
      ...
      difftestArchEvent.io.intrNO := RegNext(
        RegNext(Mux(raiseIntr && io.instrValid && valid, intrNO, 0.U))
      )
      difftestArchEvent.io.cause := RegNext(
        RegNext(Mux(raiseException && io.instrValid && valid, exceptionNO, 0.U))
      )
      difftestArchEvent.io.exceptionPC := RegNext(
        RegNext(SignExt(io.cfIn.pc, XLEN))
      )
